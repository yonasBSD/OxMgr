# Documentation

Project docs live here.

## Guides

- [Architecture Overview](./ARCHITECTURE.md)
- [User Guide](./USAGE.md)
- [Installation Guide](./install.md)
- [CLI Reference](./CLI.md)
- [Terminal UI Guide](./UI.md)
- [Latest Benchmark Results](../BENCHMARK.md)
- [Latest Benchmark JSON](../benchmark.json)
- [Pull, Webhook, and Metrics Guide](./PULL_WEBHOOK.md)
- [Deployment Guide](./DEPLOY.md)
- [Benchmark Guide](./BENCHMARKS.md)
- [Download Metrics](./DOWNLOAD_METRICS.md)
- [Service Bundles](./BUNDLES.md)
- [Oxfile Documentation](./OXFILE.md)
- [Why Oxfile vs PM2 Ecosystem](./OXFILE_VS_PM2.md)
- [Release Automation](./RELEASE.md)

## Config Examples

- [Minimal Oxfile](./examples/oxfile.minimal.toml)
- [Web Stack Oxfile](./examples/oxfile.web-stack.toml)
- [Profiles Oxfile](./examples/oxfile.profiles.toml)
- [Monorepo Oxfile](./examples/oxfile.monorepo.toml)
- [Idempotent Apply Oxfile](./examples/oxfile.apply-idempotent.toml)
